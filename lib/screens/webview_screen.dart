...
class _WebViewScreenState extends State<WebViewScreen>
  ...
  static const platform = MethodChannel('com.closh.delivery1/geolocation');
  ...
  Future<void> _showOverlayWindow() async {
    ...
    await FlutterOverlayWindow.showOverlay(
      enableDrag: true,
      overlayTitle: " Closh Delivery PartnerOverlay",
  ...
  void _listenToOverlayMessages() {
    ...
    _overlaySubscription = FlutterOverlayWindow.overlayListener.listen((data) {
      ...
      if (data == "OPEN_APP") {
        debugPrint(
            '🚀 Opening application from overlay using MethodChannel...');
        try {
        ...
        if (data is String && data.startsWith('{')) {
          ...
          if (action == 'ACCEPT_ORDER') {
            ...
            _webViewController?.evaluateJavascript(
                source:
                    "if(window.onOrderAccept) window.onOrderAccept('$orderId'); else console.log('Accepting order $orderId');");
            ...
            _webViewController?.evaluateJavascript(
                source:
                    "if(window.onOrderReject) window.onOrderReject('$orderId'); else console.log('Rejecting order $orderId');");
          }
        }
      } catch (e) {
  ...
  Future<void> _toggleOverlay() async {
    ...
    if (_lastResumeTime != null &&
      ...
      debugPrint(
          '🚫 Ignoring _toggleOverlay call: App just resumed, likely a ghost tap.');
      return;
    }

    debugPrint(
        '🔘 _toggleOverlay called. Current _isTrackingEnabled: $_isTrackingEnabled');
    if (_isTrackingEnabled) {
        ...
        if (!isServiceEnabled) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: const Text(
                    'GPS is off. Please turn it on to start tracking.'),
  ...
  Future<void> _saveFCMTokenIfPhoneAvailable() async {
    try {
      ...
      if (phoneNumber != null && phoneNumber.isNotEmpty) {
        ...
        debugPrint('⚠️ Phone number is empty, cannot save FCM token');
      }
    } catch (e) {
      ...
      const script = r"""
        (function() {
          ...
          window.fetch = async function(url, options) {
            ...
            // If the URL is a data URL (e.g. data:image/jpeg;base64,...), intercept and return a local Response
            // to bypass the site's CSP (Content Security Policy) restrictions.
            if (typeof urlString === 'string' && urlString.startsWith('data:')) {
              try {
                ...
                var bytes;
                if (isBase64) {
                  var decoded = atob(dataStr);
                  bytes = new Uint8Array(decoded.length);
                  for (var i = 0; i < decoded.length; i++) {
                    bytes[i] = decoded.charCodeAt(i);
                  }
                } else {
                  var decoded = decodeURIComponent(dataStr);
                ...
                console.error('Error intercepting data URL fetch:', e);
              }
            }

            var isLogin = urlString.toLowerCase().includes('login') || 
                          urlString.toLowerCase().includes('verify') ||
                          urlString.toLowerCase().includes('auth') ||
                          urlString.toLowerCase().includes('token');
          ...
          if (args.isNotEmpty) {
            try {
              ...
              if (body != null && body is Map) {
                debugPrint('🔍 Parsing response body for token and phone...');

                // 1. Extract Access Token
                String? accessToken;

                // Try different common locations for the token
                if (body['token'] != null) {
                ...
                } else if (body['result'] != null && body['result'] is Map) {
                  ...
                  if (dataObj['token'] != null)
                    accessToken = dataObj['token'].toString();
                  else if (dataObj['access_token'] != null)
                    accessToken = dataObj['access_token'].toString();
                  else if (dataObj['accessToken'] != null)
                    accessToken = dataObj['accessToken'].toString();
                }
                ...
                if (accessToken != null && accessToken.isNotEmpty) {
                  ...
                  // 2. Extract User Phone
                  String? phone;

                  // Try different common locations for the phone number
                  if (body['user'] != null && body['user'] is Map) {
                    phone = body['user']['phone']?.toString() ??
                        body['user']['phoneNumber']?.toString() ??
                        body['user']['mobile']?.toString();
                  } else if (body['result'] != null && body['result'] is Map) {
                    final resultObj = body['result'];
                    if (resultObj['delivery'] != null &&
                        resultObj['delivery'] is Map) {
                      phone = resultObj['delivery']['phone']?.toString() ??
                          resultObj['delivery']['phoneNumber']?.toString() ??
                          resultObj['delivery']['mobile']?.toString();
                    } else if (resultObj['user'] != null &&
                        resultObj['user'] is Map) {
                      phone = resultObj['user']['phone']?.toString() ??
                          resultObj['user']['phoneNumber']?.toString() ??
                          resultObj['user']['mobile']?.toString();
                    } else if (resultObj['phone'] != null) {
                      phone = resultObj['phone'].toString();
                    } else if (resultObj['mobile'] != null) {
                      phone = resultObj['mobile'].toString();
                    }
                  } else if (body['data'] != null && body['data'] is Map) {
                    final dataObj = body['data'];
                    if (dataObj['user'] != null && dataObj['user'] is Map) {
                      phone = dataObj['user']['phoneNumber']?.toString() ??
                          ...
                          dataObj['user']['mobile']?.toString();
                    } else if (dataObj['phone'] != null) {
                      phone = dataObj['phone'].toString();
                    } else if (dataObj['mobile'] != null) {
                      phone = dataObj['mobile'].toString();
                    }
                  } else if (body['phone'] != null) {
                    phone = body['phone'].toString();
                  } else if (body['mobile'] != null) {
                    phone = body['mobile'].toString();
                  }
                  ...
                  if (phone != null) {
                    ...
                    debugPrint(
                        '⚠️ Could not find phone number in response body');
                  }

                  // Trigger FCM Token Save
                  debugPrint('🔔 Triggering saveFCMTokenIfPhoneAvailable...');
                  await _saveFCMTokenIfPhoneAvailable();
                } else {
                  debugPrint('⚠️ Could not find access token in response body');
                }
              } else {
                debugPrint('⚠️ Response body is null or not a Map');
              }
            } catch (e) {
                    ...
                    InAppWebView(
                      ...
                      onWebViewCreated: (controller) async {
                        ...
                        controller.addJavaScriptHandler(
                          ...
                          callback: (args) async {
                            debugPrint('🟢 openCamera JS handler triggered');
                            try {
                            ...
                            return {
                              'success': false,
                              'error': 'User cancelled or failed to capture',
                            };
                          },
                        );
